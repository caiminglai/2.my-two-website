...
| 匹配算法 | 关键词精确匹配 | 向量语义匹配 + 关键词 | 匹配质量大幅提升 |
| 数据库扩展性 | SQLite 单文件 | PostgreSQL 集群可扩展 | 架构升级 |

### 1.5 向量数据库架构升级（已完成）

数据库从 SQLite 迁移到 PostgreSQL 18 + pgvector v0.8.2
- 数据目录迁移到 `data/pg_data/`
- 新增 `user_vectors` 向量表，存储用户画像和择偶要求向量
- HNSW 索引 + 余弦相似度，支持高效的近似最近邻搜索
- 新增 `db/vector.js` - 向量数据库操作模块（8个函数）
- 新增 `services/vector.service.js` - 向量业务逻辑层
- 新增 `routes/vector.js` - 向量搜索 API 路由（5个接口）
- 支持资料相似搜索、要求相似搜索、双向匹配推荐三种模式
- 向量维度：1536 维（OpenAI text-embedding-ada-002）
- 支持 mock 模式，开发环境无需 API Key 即可测试
...
4. 向量数据迁移：现有用户需批量生成向量数据
5. 向量模型升级：未来可考虑替换为更高质量的 embedding 模型
6. PostgreSQL 性能调优：需根据实际数据量进行参数调优
