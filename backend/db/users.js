...
function setAdminPassword(password) {
  const database = getDb();
  const stmt = database.prepare('INSERT OR REPLACE INTO admin_settings (key, value, updated_at) VALUES (?, ?, ?)');
...
function addAdminLog(action, targetId, details, ipAddress) {
  ...
  stmt.run(action, targetId || null, details || null, ipAddress || null, Date.now());
}
